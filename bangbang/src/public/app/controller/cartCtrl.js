...
(function() {
  ...
  TigerController.controller('cartCtrl',function($scope,$cookieStore,$ionicViewSwitcher,$ionicHistory,$stateParams,$state,globalService,publicFunction,$ionicPopup) {
    ...
    $scope.request = {
      findCartList : function(){
        //购物车列表
        globalService.commonGet(TigerConfig.api.findCartList+"?appId="+TigerConfig.appId+"&userId="+$scope.user.uid).then(function(result){
          if(result.code == 0){
      ...
      updateNumber : function(goods){
        globalService.commonGet(TigerConfig.api.updateNumber+"?id="+goods.id+"&number="+goods.goodsnum).then(function(result){
      ...
      cartDel : function(param){
        $ionicPopup.confirm({
          ...
          globalService.commonPost(TigerConfig.api.cartDel,param).then(function(result){
            console.log("删除选中商品：：");
            console.log(result);
            if(result.code == 0){
