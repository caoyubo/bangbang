...
(function() {
  ...
  TigerController.controller('registerCtrl',function($scope,$cookieStore,$interval,$http,$stateParams,$ionicViewSwitcher,$state,globalService,$cordovaBarcodeScanner) {
    ...
    $scope.getRcode = function(){
      ...
      globalService.commonPost(TigerConfig.api.getRcodePost,params).then(function(result){
        if(result.r == 1){
          console.log(result);
          countdown();
        }else{
    ...
    $scope.register = function(userForm){
      ...
      if(userForm.$valid){
        ...
        if($scope.favor.isChecked){
          ...
          globalService.commonPost(TigerConfig.api.registerPost,params).then(function(result){
            console.log("注册::");
            console.log(result);
            ...
            if(result.r == 1){
              ...
              globalService.commonPost(TigerConfig.api.loginPost,{username:params.username,passwd:params.passwd}).then(function(data){
                if(result.r != 1){  //失败就直接跳转到登陆页面手动登陆
                  $state.go('login');
                  return null;
                }
        ...
        func.validate(userForm,['userName','userPhone','userRcode','userPasswd','userRepasswd']);
        console.log("验证不通过!!");
      }
    }
