...
(function() {
    ...
    TigerController.controller('codeCtrl',function($scope,$cookieStore,globalService,$stateParams) {
        ...
        $scope.request = {
            getUpLevelList : function(){
                //获取当前等级以及可申请等级，积分
                globalService.commonGet(TigerConfig.api.getUpLevelList+"?uid="+$scope.user.uid).then(function(result){
                    if(result.code == '0'){
            ...
            getStatusByUid : function(){
                //根据用户id查找用户申请状态
                globalService.commonGet(TigerConfig.api.getStatusByUid+"?uid="+$scope.user.uid).then(function(result){
                    if(result.code == '0'){
