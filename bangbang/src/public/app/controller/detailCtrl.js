...
(function() {
    ...
    TigerController.controller('detailCtrl', function ($scope, $cookieStore, $stateParams, $ionicViewSwitcher, $ionicHistory, $state, $ionicModal, $ionicBackdrop, $ionicActionSheet, globalService, publicFunction,$ionicPopup) {
        ...
        $scope.request = {
            getGoodsDetail: function () {
                globalService.commonGet(TigerConfig.api.findById + "?id=" + $scope.id).then(function (result) {
                    if (result.code == 0) {
                        console.log("商品详情信息::");
                        console.log(result);
            ...
            cartAdd: function () {
                ...
                globalService.commonPost(TigerConfig.api.cartAdd, param).then(function (result) {
                    if (result.code == 0) {
                        console.log(result);
                        $ionicPopup.alert({
                            title : '加入购物车成功！',
                ...
                if (specList[key].id == specId) {
                    ...
                    for (var i in specAttrList) {
                        if (specAttrList[i].id == specAttrId) {
                            ...
                            $scope.currSpecInfo = specAttrList[i];
                        } else {
