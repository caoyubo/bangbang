...
(function() {
    ...
    TigerController.controller('categoryCtrl',function($scope,$ionicModal,$ionicViewSwitcher,$cookieStore,$state,globalService,publicFunction) {
        ...
        $scope.request = {
            findSecAndThrClass : function(classId){
                globalService.commonGet(TigerConfig.api.findSecAndThrClass+"?goodsclassid="+classId).then(function(result) {
                    if (result.code == 0) {
                        /*console.log("获取二三级分类:");
            ...
            findFirstClass : function(){
                globalService.commonGet(TigerConfig.api.findFirstClass).then(function(result){
                    if(result.code == 0){
